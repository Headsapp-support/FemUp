...
const postuler = async (req, res) => {
  try {
    console.log("Données reçues pour la candidature : ", req.body); // Affiche les données reçues

    const { offerId } = req.body; // Récupère uniquement l'offerId dans la requête (le CV sera traité différemment)
    const cvUploaded = req.file; // Le fichier est récupéré via `req.file` (on suppose que multer est utilisé pour gérer le téléchargement du fichier)
    ...
    console.log("Candidat trouvé : ", condidat); // Affiche le candidat

    // Vérification si le candidat a déjà postulé à l'offre
    ...
    // Recherche du recruteur et de l'offre
    ...
    // Vérification de l'ID de l'offre dans le tableau des offres du recruteur
    ...
    // Ajouter la candidature à la liste des candidatures
    condidat.applications.push({
      jobId: offer._id,
      Offre: offer.title,
      status: 'En attente',
      date: new Date(),
      cvUploaded: cvUploaded.path // Utilise le chemin du fichier téléchargé (ou une autre propriété pertinente)
    });

    // Sauvegarder la candidature du candidat
    await condidat.save();
    ...
    console.log("Candidature enregistrée : ", condidat);
    console.log("Offre mise à jour : ", offer);

    res.status(200).json({ success: true, message: 'Candidature envoyée avec succès' });
  } catch (error) {
    console.error('Erreur lors de l\'envoi de la candidature:', error); // Log l'erreur
    res.status(500).json({ message: 'Erreur serveur', error: error.message });
  }
};
...
const getCandidatures = async (req, res) => { 
  try {
    const condidat = await Condidat.findById(req.user.id)
      .populate({
        path: 'applications.jobId',  // Peupler le champ 'jobId' pour obtenir les informations de l'offre
    ...
    // Log des candidatures et des jobs
    console.log('Candidatures:', condidat.applications);
