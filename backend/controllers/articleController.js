...
const createArticle = async (req, res) => {
  ...
  const getArticleById = async (req, res) => {
    try {
      const article = await Article.findById(req.params.id); // Récupère l'article en fonction de l'ID passé dans l'URL
      if (!article) {
  ...
  const getRelatedArticles = async (req, res) => {
    try {
      const articles = await Article.find({}).limit(5); // Récupère les 5 articles les plus récents (ou similaires selon ta logique)
      res.json(articles);
    } catch (error) {
